...
4. **FEC — optional DT tuning:** Linux **`fsl,fec`** binding and **`fec_enet_parse_rgmii_delay()`** allow **only `0` or `2000`** ps per property (not arbitrary values). Hardware applies **`FEC_ENET_TXC_DLY` / `FEC_ENET_RXC_DLY`** only when **`FEC_QUIRK_DELAYED_CLKS_SUPPORT`** is set (e.g. **`fsl,imx8qm-fec`**); **`fsl,imx8mq-fec`** / **`fsl,imx8mm-fec`** match **`fec_imx8mq_info`**, which **does not** set that quirk in mainline — so parsed DT props **do not turn on MAC-side delay lines** on typical **i.MX8MM** builds. Confirm in your kernel tree’s **`fec_main.c`** if a vendor patch adds the quirk for **8MM**.
