...
| B3 | **CP2108** (U13 quad USB‑UART / `cp210x`) | **Validated (lab 2026-05-16):** **[0],[1]** RS‑232; **[2],[3]** RS‑485 + **`GPIO.10`/`GPIO.14` DE** with NVM **`EnhancedFxn_IFC2/3 = 0x0c`** (**`cp2108-set-portconfig --rs485-de-invert`**); DE high on TX confirmed (scope). **`cp2108-get/set-portconfig`** + **`rs485_tx_bytes`** in **`board-scripts`**. **Production:** one-time NVM per unit — checklist § CP2108; alt. **[AN721](https://www.silabs.com/documents/public/application-notes/AN721.pdf)** / Xpress. **`QUART_RES#`** = **GPIO4_IO5**. |
