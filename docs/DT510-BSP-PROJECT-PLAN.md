...
| ID | Task | Notes |
|----|------|--------|
| B1 | **BQ25792** (charger, I2C3 `0x6B`, `CHGR_INT#`) | **Done (DT + lab path):** `battery-dt510` + `bq25792@6b` **okay**; **`bq25792-charger.cfg`** documents kernel gap (no **bq257xx** in **6.6.x** fslc). **CHGR_INT#** wired in DTS per B1 SSOT (GPIO4_IO9). Lab: **`i2c-dev`** + **`i2c-2`** (I2C3). **Kernel identity (read this):** DT510 factory images use **`linux-lmp-fslc-imx`** → **`Freescale/linux-fslc`** branch **`6.6-2.2.x-imx`**; **`meta-lmp`** pins **`SRCREV_machine`** (verify with **`bitbake -e linux-lmp-fslc-imx \| grep SRCREV`** — do not assume a doc example SHA). **Prerequisite for BQ25792 driver work:** mainline commit **`3b1bbfb5fce3`** (*mfd: bq257xx: Add support for BQ25703A*). **Cherry-pick test (2026-04-14)** onto branch tip **`97812d71`** / **6.6.129:** applies with **one `drivers/mfd/Makefile` conflict** (trivial: add **`obj-$(CONFIG_MFD_BQ257XX)`** line in sorted order). Lore **BQ25792** series applies **after** that stack. See issue **#3**. |
