...
| Charger **BQ25792** | I2C3 `0x6B`, `CHGR_INT#` | **partial** | **`bq25792@6b` enabled** + `simple-battery`; **`bq25792-charger.cfg`** + **§5 Tier B1** in project plan — factory kernel is **`linux-lmp-fslc-imx`** / **`linux-fslc`** **`6.6-2.2.x-imx`** (pin **`SRCREV`** via **`bitbake -e`**); prerequisite mainline **`3b1bbfb5fce3`** (BQ25703A **bq257xx** stack) before lore **BQ25792** patches; cherry-pick test 2026-04-14: **Makefile** conflict only. **CHGR_INT#** in DTS (GPIO4_IO9). Lab: **`i2c-dev`** on **`i2c-2`** (I2C3). **Issue #3.** | B1 |
