...
| **Charger / HDMI placeholders** | Not in the DT510-vs-Sentai DTS diff as matching disabled nodes | **`bq25792@6b` enabled** (Tier B1 — kernel charger TBD); **`lt9611@39` staged** (DSI/HDMI graph + `lt9611-hdmi.cfg`) but **still disabled** — enable after I²C @ `0x39` + 1.8 V VDD confirm |
...
| Mic **TAA5412** | I2C2 `0x51` | partial | DTS: **`taa5412@51`**, **`&sai5`** + **`pinctrl_sai5_taa5412`**, **`sound-taa5412`**; **`&micfil` disabled**. **Driver (one per image):** Path A **`taa5412`** → **`snd_soc_pcm6240`** + **`taa5412-i2c-1-1dev.bin`**; Path B **`taa5412-tac5x1x-ti`** (factory subscriber) → **`snd_soc_tac5x1x_taa5412`** OOT. **ALSA:** **`arecord -D driver_mic`** only for **SAI5** scope — **`aplay -D driver_speaker`** is **SAI3/TAS2563**, not mic. **Lab 2026-05-06** (factory **307**): card missing — **`&micfil`** grabbed **SAI5_RXC** (fixed in DTS). **422+:** card **`taa5412codec`**, stream opens; if scope dead during **`arecord`**, see [**`DT510-TAA5412-DRIVER-MIC-ALSA.md`**](DT510-TAA5412-DRIVER-MIC-ALSA.md) § Michael bench. **IN1/IN2 differential:** [§ Codec notes](#codec--amp--dt-vs-hardware-engineering-notes-2026-05-06) | C2 |
...
| HDMI **LT9611** | I2C3 — SSOT `0x72` (8-bit) → DT **7-bit `0x39`** | **staged / disabled** | DTS: `hdmi-connector` + `lt9611@39` **ports** (DSI in / HDMI out) + `&mipi_dsi` ports (4 lanes); `reg_lt9611_vdd_1v8` stub; EVK `hdmi@3d` **deleted**. Kernel: **`lt9611-hdmi.cfg`** → `CONFIG_DRM_LONTIUM_LT9611=m`. Nodes stay **disabled** until fitted BOM: I²C ACK @ `0x39`, confirm 1.8 V `vdd` parent, then flip `lt9611` + `&mipi_dsi` to okay. | C3 |
| Auth **SE050** | I2C4 `0x48`; SoC **D13/E13** | **aligned with stack** | OpTEE **`CFG_CORE_SE05X_I2C_BUS=3`** = **`&i2c4`** (same as Sentai). **Not** SAI1_RXD2/RXD3 — see [`DT510-SE050.md`](DT510-SE050.md). Machine `se05x` + OEFID set. | B4 |
...
| Ethernet **KSZ9896** | ENET RGMII + **I²C mgmt** (`&i2c3`/`i2c-2` @ `0x5f`) | validated (healthy HW) | **`&fec1`** RGMII `fixed-link` CPU port + **DSA** user ports **`lan1..lan4`**; switch managed over **I²C** (`microchip,ksz9896`, driver-bound → **`UU`** at `0x5f`), **`reset-gpios`** GPIO4_IO1. **`/delete-node/ mdio`** under **`&fec1`**. Validate with **`dt510-ksz9896-check.sh`** (production-test step 6) — see **`docs/DT510-ETHERNET-KSZ9896.md`**. | C1 |
| GNSS **NEO-M9V** | UART via **CP2102N** USB + GPIO reset (`gnss-res#`) | **validated (lab); regression open 2026-06-10** | **2026-05-05/18:** NMEA + NDTR **`/dev/gnss`** @ **38400**. **2026-06-10 (`.205`):** symlink OK, **no NMEA**, **TXD ~2.5 V** idle. **BSP:** drop **`gnss-res-hog`**, **`gpio-line-names`** IO29, **`dt510-gnss-reset-pulse`**. Tools: **`MACHINE_FEATURES` `neo-m9v`** → **`gpsd`** / **`gps-utils`** / **`gps-utils-python`** (`ubxtool`; daemon auto-start off). Chain: NEO-M9V → CP2102N **`10c4:ea60`**. | C5 |
| HDMI misc **HDMI2C1-6C1** | GPIO | partial | Fault line per SSOT — align with LT9611 bring-up | C3 |
| **CP2108** quad-UART (U13, USB HS) | **Internal UART [0–3]** (not `ttyUSBn` order) | **validated (lab)** | **2026-05-16 lab (Michael):** all four UARTs working — **[0]/[1]** **RS-232** (`ttyUSB0`/`ttyUSB1`, IFC **`0x00`**); **[2]/[3]** **RS-485** after NVM **`EnhancedFxn_IFC2`/`IFC3` = `0x0c`** (`cp2108-set-portconfig --rs485-de-invert`); **DE high on TX**, low idle (scope on **`RS485_DE1`/`DE2`**); **`ttyUSB2`/`ttyUSB3`** (**`rs485_tx_bytes`**). **`cp210x`** + **`cp2108-get/set-portconfig`** (**`board-scripts`** / **`cp2108-usb-serial`**). **Port map (O.H.):** **[2]** `RS485_TX1`/`RX1` + **GPIO.10→`RS485_DE1`**; **[3]** `RS485_TX2`/`RX2` + **GPIO.14→`RS485_DE2`**. **Production:** one-time NVM program per unit (§ **CP2108** below). SoC **`QUART_RES#`**: **SAI1_RXD3 → AF17 → GPIO4_IO5** (not I2C4). **`/dev/serial/by-path`**. | B3 |
| Digital I/O | GPIO1_IO0–9 | **validated (lab + fleet)** | **`pinctrl_gpio1_dio_in`** (DI → GPIO1_IO0/1/4/5) + **`pinctrl_gpio1_dio_out`** (DO → IO6–9); EVK **`ir_recv` / `reg_pcie0` / `backlight`** disabled. **SW_PAD (`fsl,pins` 2nd cell, IMX8MMRM Ch.8 — see `imx8mm-sw_pad_ctl-fields.h`):** DI **`0x010`** (**`PE_DIS`**, FSEL fast, **`DSE_X1`**) — no SoC bias; DO **`0x116`** (**`GPIO_STD`**: PE + internal pull-down). Board adds external terminations where required. **Andy/Vix:** cab DI polarity via **`&gpio1` `gpio-line-config`** offsets **0, 1, 4, 5** (`GPIO_PULL_UP \| GPIO_ACTIVE_LOW`); apps **`active_low=False`** (invert in DTS only). **2026-05-21 (Andy):** DI1 call-request, DI2 emergency, DI3 PTT correct on Foundries **415** / **`f4e1f61`**. Scripts when **`dt510-digital-io`**: **`dt510-dio-toggle-outputs`** (DO), **`dt510-dio-poll-inputs`** (DI) + **`libgpiod-tools`**. Lab **2026-05-08**: **DO** toggle and **DI** paths (**O.H.**). **Cab buttons:** § below. | B2 |
...
- **DTS covers:** **`taa5412@51`**, **`sound-taa5412`**, **`I²S`**, **`&sai5`** CPU clock master, **`pinctrl_sai5_taa5412`**, **`pinctrl_taa5412_codec_gpio`** (**`GPIO4_IO18`**), **`fsl,sai-mclk-direction-output`**, **`/delete-property/ fsl,sai-asynchronous`**, **`fsl,sai-synchronous-rx`** (LRCK on **`SAI5_TX_SYNC`** / **`SAI5_RXD1`**). Early **`lmp-350`** failed when **both** async + sync-Rx were set — current DTS deletes inherited async first. **`&micfil` disabled** so **SAI5_RXC** is not grabbed by EVK PDM.
...
- **DTS / upstream binding do not set:** differential vs single-ended **mic** routing for IN1/IN2. **Path A:** TI **`.bin`** firmware via **`snd_soc_pcm6240`**. **Path B:** OOT **`tac5x1x`** register init + DAPM patches (**`0003`/`0004`**). Validate **analog mode + levels** against schematic, not DTS alone.
- **Revisit when:** switching **`taa5412`** ↔ **`taa5412-tac5x1x-ti`**, PCM6240 patch series, SAI5 DT flags, or capture topology.
...
- **Userspace ALSA (2026‑05):** **`alsa-state`** installs **`/etc/asound.conf`** with **`tannoys`** + **`tannoy_slot2`/`slot3`** + **`tas6424-init`**. **`amixer -D tannoys`** **`Tannoy CH1`–`CH4`** = **passenger tannoy** levels (kernel rename from TI **“Speaker Driver CHn”**); **not** cab **driver** amp — driver is **TAS2563** / **`drivers`** / **`driver_speaker`**. Canonical reference: [`DT510-TAS6424-TANNOY-ALSA.md`](DT510-TAS6424-TANNOY-ALSA.md).
...
**Reset:** SoC hog **`QUART_RES#`** on **GPIO4_IO5** (**SAI1_RXD3 / AF17** — **`quart-res-hog`** in `imx8mm-jaguar-dt510.dts`; **not** I2C4).

---

## Cab buttons (DT510)

Cab **call-request**, **emergency**, and **driver PTT** use the **DI header** on **GPIO1** (`gpiochip0`), not **`/dev/input`** **`gpio-keys`**. **`vix-apps`** containers poll via **libgpiod** (`/dev/gpiochip0` passed through compose).

| DI header | SoC pad | `gpiochip0` offset | Line name | App |
|-----------|---------|-------------------|-----------|-----|
| DI1 | GPIO1_IO0 | 0 | call-request | **NDTR** (`button_1` → `ACT_CALL`) |
| DI2 | GPIO1_IO1 | 1 | emergency | **NDTR** (`button_2` → `ACT_CALL_EM`) |
| DI3 | GPIO1_IO4 | 4 | ptt | **AVM** (driver PTT → **`TaskType.DRIVER_PA`**) |
| DI4 | GPIO1_IO5 | 5 | dio-input-4 | spare |
...
| **Containers** | **`dt510_gpio.py`**: **`LineSettings(active_low=False)`**, **`is_pressed()`** when libgpiod **ACTIVE** (**1**) — NDTR + AVM; **vix-apps ≥ `f4e1f61`** |

**Do not** set both DTS **`GPIO_ACTIVE_LOW`** and **`LineSettings(active_low=True)`** (double invert — pressed/released swapped).

### Verification

**Status: validated (lab + fleet)** — cab buttons **DI1–DI3** in **NDTR** + **AVM**; libgpiod v2 **`get_value`**; single invert (**DTS + `active_low=False`**, **`f4e1f61`**, containers **415**).

| When | Where | Result |
|------|--------|--------|
| **2026-05-19** | Lab | libgpiod v2 path; call/emergency/PTT with bench harness |
| **2026-05-21** | Fleet (**Andy**) | DI1 call-request, DI2 emergency, DI3 PTT — correct polarity after OTA **415** |
...
**Software:** libgpiod v2 **`get_value`** (containers **≥393**). Polarity: **DTS** **`GPIO_ACTIVE_LOW`** only; **`dt510_gpio.py`** **`active_low=False`** (**`f4e1f61`**).

### GPIO ownership (one consumer per line)

| Lines | Consumer | Container |
|-------|----------|-----------|
| **0–1** (call-request, emergency) | **`ndtr-switch-monitor`** | **`vix-apps-ndtr-1`** |
| **4** (PTT) | **`avm-ptt`** | **`vix-apps-avm-1`** |

A second **`gpiod.request_lines`** on offsets **0** or **1** while NDTR is running returns **`EBUSY`** (expected). Do not run a host tool that **claims** DI1/DI2 at the same time as NDTR.

### Press semantics (call-request / emergency)

On **button release**, NDTR compares hold time to **`LONG_PRESS_DURATION_SECS`** (**3.0** s in **`NDTR/common/utils.py`**):

- **Short press** → enqueue normal or emergency call-request (if queue empty).
- **Long press** → **no** call-request enqueue (same rule for emergency).

### Engineering SSH (NDTR diagnostics)

- **Port:** **`data_port + 1`** → **9101** when **`data_port = 0.0.0.0:9100`** (see **`docker-compose.yml`**).
- **Login:** **`vix`** / **`vixdt`** (lab default in **`NDTR/config*.txt`** — change for production).
- **Does not stream live button edges.** Use **`docker logs`** on the board in **parallel** while pressing buttons or while using **`gpio set`** to simulate.

| Command | Purpose |
|---------|---------|
| **`gpio info`** | DI state / switch-monitor summary |
| **`gpio set <pin> <action>`** | Simulate header pin **1–11** (not 5): **`0`** released, **`1`** pressed, **`2`** toggle without press/release semantics |
| **`callrequest <N\|E> <phone…>`** | Enqueue call without hardware ( **`N`** normal, **`E`** emergency ) |

Implementation: **`NDTR/diagnostics/cmd_messaging.py`** (`CmdGpio`, `CmdCallRequest`).

### Log strings (grep / follow)

**NDTR** (`docker logs -f vix-apps-ndtr-1`):

| String | When |
|--------|------|
| **`call request`** | INFO — call-request **pressed** |
| **`Enqueuing call request event`** / **`Enqueuing emergency call request event`** | INFO — short press released, queued |
| **`Detected activity on GPIO pin`** | DEBUG — edge from switch monitor |
| **`Call Request button: PRESSED` / `RELEASED`** | DEBUG |
| **`Emergency Call Request button: PRESSED` / `RELEASED`** | DEBUG |
| **`Call Request button was short pressed`** / **`long pressed - NOT generating`** | DEBUG — release timing |

**AVM** (`docker logs -f vix-apps-avm-1`): little PTT-specific logging; follow **`DRIVER_PA`** / driver-PA queue activity in **`button_monitor.py`** / **`audio_scheduler.py`**.

### Bench (host + containers)

1. **Raw DI levels (host, does not claim lines for NDTR):**  
   `sudo dt510-dio-poll-inputs` or `sudo dt510-dio-poll-inputs --once`  
   ( **`board-scripts`**, feature **`dt510-digital-io`** ). **`gpioget --numeric`** is **raw** pad level (pressed often **0**, released **1** with pull-up wiring). App/container **`is_pressed`** uses kernel **ACTIVE** (**1** = pressed) when DTS **`GPIO_ACTIVE_LOW`** + **`active_low=False`**; if inverted in logs, check containers **`f4e1f61`** not **`cc26937`**.
...
4. **Simulate without panel:** engineering SSH → **`gpio set 1 1`** then **`gpio set 1 0`** (DI1 short press).

AVM PTT notes: **`vix-apps/AVM/VIX_HANDOFF_AVM_DEBUGGING.md`** (§ Cab buttons).
...
2. **TAA5412** @ I2C2 **`0x51`** / **SAI5** — Path A/B drivers landed; **`arecord -D driver_mic`** for capture/scope (**not** **`driver_speaker`**). See [**`DT510-TAA5412-DRIVER-MIC-ALSA.md`**](DT510-TAA5412-DRIVER-MIC-ALSA.md) § Michael bench.
...
5. **Tier C3:** LT9611 staged (graph + driver CFG). Next: I²C @ `0x39` on fitted board, confirm 1.8 V VDD net / reset polarity, then `status = "okay"` on `lt9611` + `&mipi_dsi`.
