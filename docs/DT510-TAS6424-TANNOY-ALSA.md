...
| **Userspace ALSA** | **Working:** named **`pcm`/`ctl` `tannoys`**, **`tannoy_slot2`/`slot3`**, **`tannoy_both_mono`/`tannoy_both_lr`**, **`tannoy_all`** — lab **`aplay`** validation. **`plug`** wrappers handle typical **rates/formats**; raw **`pcm.tannoys`** is **`hw`** (match params or use **`plug:tannoys`**). |
...
| **`tannoy_slot2` / `tannoy_slot3`** | **One** Tannoy each — map to **IEC indices 2 and 3**. |
