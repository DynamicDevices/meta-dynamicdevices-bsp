...
4. **DT510 pop mitigation (2026‑06) — `PRE_POWER_UP` order:** page select → **`02=03`** (VREF, **15 ms**) → page **1** **`73=d0`** (MICBIAS, **15 ms**) → **`78=a0`** (PWR, **15 ms**) → **`50/55/5a/5e=00`** (AC, **5–10 ms** each) → **`72=28`** (HPF 12 Hz, **10 ms**) → **`76=c0`** (CH_EN, **5 ms**) → **`77=c0`**. **Does not write Ch1 Digi (`0x52`)** — **`taa5412-init`** / AVM own gain; capture-open pop is **`pcm6240` `0008`** (PRE_POWER before ASI) + stagger/HPF, not a regbin Digi mute. Factory target **543** still ships the prior regbin with **`52=00`** until the next manifest pin. Regenerate with **§ A′** after JSON edits.
