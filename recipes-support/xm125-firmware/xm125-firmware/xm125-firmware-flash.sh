...
xm125_wake_assert() {
    ...
    gpioset --chip "${XM125_GPIO_CHIP}" "${XM125_WAKE_LINE}=1"
}

xm125_wake_deassert() {
    log "Deasserting XM125 wake up (low)"
    log "DEBUG: gpioset ${XM125_GPIO_CHIP} ${XM125_WAKE_LINE}=0"
    gpioset --chip "${XM125_GPIO_CHIP}" "${XM125_WAKE_LINE}=0"
}
...
xm125_wait_for_ready() {
    ...
    while [ $count -lt $timeout ]; do
        ...
        local mcu_int_status=$(gpioget --chip "${XM125_GPIO_CHIP}" "${XM125_IRQ_LINE}")
